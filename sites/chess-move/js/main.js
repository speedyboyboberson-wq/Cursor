(function () {
  ...
  function isOpenAt(now) {
    const hours = M.hours || {};
    const spec = hours[String(now.getDay())];
    if (!spec) return false;
    const mins = now.getHours() * 60 + now.getMinutes();
    return mins >= spec[0] && mins < spec[1];
  }
  document.querySelectorAll("[data-open-status]").forEach((el) => {
    ...
    if (label) label.textContent = open ? (M.openText || "Open now") : (M.closedText || "Currently closed");
  });
  ...
  function estimateFrom(formEl) {
    ...
    if (sameDay) total *= Number(M.sameDay || 1);
  ...
  function renderEstimate(formEl) {
    ...
    if (M.quoteOnly || !RATE) {
      ...
      if (detail) detail.textContent = "Call for a number — no public hourly";
      return;
    }
    const { hours, sameDay, total } = estimateFrom(formEl);
    out.textContent = "$" + Math.round(total).toLocaleString("en-US");
    detail.textContent = (sameDay ? `${hours} hrs × $${RATE} · same-day multiplier ${M.sameDay}` : `${hours} hrs × $${RATE} · 2 movers · 2-hour minimum`);
  }
  if (form) {
    ...
    form.addEventListener("submit", (e) => {
      ...
      const { total } = RATE ? estimateFrom(form) : { total: 0 };
      const body = `Name: ${name}\nPhone: ${phone}\nEmail: ${email || "n/a"}\nFrom: ${from}\nTo: ${to}\nDate: ${date || "flexible"}\nMove size: ${size}\nService: ${service}\nHours: ${hours}\nTiming: ${timing}\nEstimate: ${RATE ? "$"+Math.round(total) : "quote"}\n\n${message}`;
      if (M.email) {
        const mailto = `mailto:${M.email}?subject=${encodeURIComponent("Move booking request")}&body=${encodeURIComponent(body)}`;
        if (note) note.textContent = "Opening your email. You can also use Contact.";
        window.location.assign(mailto);
        return;
      }
      if (note) note.textContent = "Request ready. Use Call or the marketplace link in Contact if a phone/email is not listed.";
      try { navigator.clipboard.writeText(body); } catch (err) {}
      if (M.phone) window.location.assign("tel:" + M.phone);
      else if (M.bookUrl) window.open(M.bookUrl, "_blank", "noopener");
    });
  }
